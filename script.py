...
def save_result(user_id, username, profession):
    conn = sqlite3.connect("career_results.db")
    cursor = conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO career_results (user_id, username, profession) VALUES (?, ?, ?)",
                   (user_id, username if username else "Не указан", profession))
    conn.commit()
    conn.close()
...
def finish_career_test(user_id: int):
    ...
    user_current_menu.pop(user_id, None)
